...
com.logicpartners.propertyInspector = function (designer, canvas) {
	...
	this.update = function (activeElement) {
		...
		if (this.activeElement == activeElement) {
			...
			// Add angle buttons if the element is a text object
			if (activeElement.text !== undefined) {
				var angleButtonContainer = $('<div></div>')
					.css({
						"clear": "both",
						"padding-top": "2px",
						"margin-left": "5px",
				...
				var angleLabel = $('<div>Rotation</div>')
					.css({
						"width": "65px",
						"height": "20px",
				...
				angleButtonContainer.append(angleLabel).append(angleButtons);
				this.propertyView.append(angleButtonContainer);
			}
